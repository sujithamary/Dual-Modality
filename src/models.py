...
class CrossAttention(nn.Module):
    ...
    def forward(self, q_x, kv_x):
        ...
        h = self.num_heads
        Q = Q.view(B, Nq, h, D//h).transpose(1,2)  
...
class DualModalityNet(nn.Module):
    ...
    def forward(self, mri, hsi):        
        B, _, H, W = mri.shape
        m_tokens = self.mri_enc(mri) 
        h_tokens = self.hsi_enc(hsi)  

        m2h, attn_m2h = self.cross1(m_tokens, h_tokens) 
        m_tokens = m_tokens + m2h
        m_tokens = self.norm_m(m_tokens)

        h2m, attn_h2m = self.cross2(h_tokens, m_tokens) 
        h_tokens = h_tokens + h2m
        h_tokens = self.norm_h(h_tokens)

        fused = torch.cat([m_tokens, h_tokens], dim=-1)
        fused = self.fuse_linear(fused)  
        out = self.decoder(fused, B, H, W) 
